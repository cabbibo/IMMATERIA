Shader "IMMAT/Debug/GridDebugID4"
{
    Properties
    { 
        _Color ("Color", Color) = (1,1,1,1)
        _Size ("Size", float) = .01
        _ParticleCount ("ParticleCount", float) = 10000
    }
    SubShader
    {
...
varyings vert (uint id : SV_VertexID){
  ...
  if( base < _Count ){
    ...
    if( alternate == 5 ){ extra = -l + u; uv = float2(0,1); }
      ...
      o.worldPos = GetPos( base )  + extra *_Size;//* min(abs((1/(20*v.dist))) * _Size,_Size);
...
float _ParticleCount;
      ...
      float4 frag (varyings v) : COLOR {
          ...
          float3 col = v.ids.xyz/_ParticleCount;// v.ids.x;// / 4096;
