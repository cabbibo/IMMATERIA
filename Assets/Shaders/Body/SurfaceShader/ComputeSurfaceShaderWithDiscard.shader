  ...
  Shader "IMMAT/Surface/SurfaceWithDiscard" {
    Properties {
      _MainTex ("Texture", 2D) = "white" {}
      _DiscardMap ("DiscardMap", 2D) = "white" {}
      _DiscardCutoff ("DiscardCutoff", Range(0,1)) = 0.5
...
// have to make sure platform supports compute shaders!
      ...
      sampler2D _DiscardMap;
      float4 _DiscardMap_ST;
...
Vert v = _VertBuffer[_TriBuffer[vert.id]];
            ...
            data.texcoord3 = TRANSFORM_TEX(v.uv , _DiscardMap);//float2(1,1);
      ...
      void surf (Input IN, inout SurfaceOutputStandard o) {
        ...
        float3 discardCol = tex2D (_DiscardMap, IN.texcoord3.xy).rgb;
        float3 nor = UnpackNormal (tex2D (_BumpMap, IN.texcoord2.xy));

        if( length( discardCol ) < _DiscardCutoff){ discard; }
